...
def parse_args():
    ...
    parser.add_argument('--exp_num', type=int, default=2, help='The experiment number')
    parser.add_argument('--phase', type=str, default='test_adobe', choices=['train', 'test', 'test_custom', 'test_adobe', 'test_adobe_metric', 'test_middlebury', 'test_snufilm' , 'metrics_evaluation', 'test_adobe_metric', 'test_stopmotion'])
    ...
    parser.add_argument('--dataset', default='Vimeo', choices=['X4K1000FPS', 'Vimeo', 'ATD12k'],
                        help='Training/test Dataset')
    parser.add_argument('--stopmotion_data_path', type=str ,default='/HDD/sujincho/AnimationDataset_triplet')
    ...
    parser.add_argument('--train_data_path', type=str, default='/HDD/dataset/X4K1000FPS/train')
    parser.add_argument('--val_data_path', type=str, default='/HDD/dataset/X4K1000FPS/val')
    #parser.add_argument('--test_data_path', type=str, default='/HDD/dataset/X4K1000FPS/test')
    parser.add_argument('--test_data_path', type=str, default='/HDD/dataset/Adobe240fps/Adobe240fps_test_spilt_XVFI')

    parser.add_argument('--atd12k_data_path', type=str, default='/HDD/sujincho/atd_12k/datasets')
    

    parser.add_argument('--vimeo_data_path', type=str, default='/HDD/dataset/Vimeo90k/vimeo_triplet')
    ...
    parser.add_argument('--num_thrds', type=int, default=2, help='number of threads for data loading')
    ...
    parser.add_argument('--custom_path', type=str, default='/HDD/sujincho/taktoon_problem/images', help='path for custom video containing frames')
...
def main():
    ...
    if args.dataset == 'Vimeo':
        if args.phase != 'test_custom' and args.phase != 'test_adobe':
            args.multiple = 2
        args.S_trn = 1
        args.S_tst = 1
    ...
    if args.phase != "metrics_evaluation" and args.phase!="test_adobe_metric":
        print("\n-------------------------------------- Final Test starts -------------------------------------- ")
    ...
    if args.dataset == 'X4K1000FPS' and args.phase == 'test_adobe_metric' and args.phase != 'test_custom':
        final_pred_save_path = os.path.join(args.test_img_dir, args.model_dir, 'epoch_' + str(epoch).zfill(5)) + postfix
        metrics_evaluation_Adobe(final_pred_save_path, args.test_data_path, args.metrics_types,
                                  flow_flag=args.saving_flow_flag, multiple=args.multiple)
        

    elif args.dataset == 'X4K1000FPS' and args.phase =='test' and args.phase != 'test_custom':
        final_pred_save_path = os.path.join(args.test_img_dir, args.model_dir, 'epoch_' + str(epoch).zfill(5)) + postfix
...
def test(test_loader, model_net, criterion, epoch, args, device, multiple, postfix, validation):
    ...
    wr = csv.writer(f)
    parameter = sum(p.numel() for p in model_net.parameters())
    ...
    with torch.no_grad():
        ...
        for testIndex, (frames, t_value, scene_name, frameRange) in enumerate(test_loader):
            ...
            if args.phase != 'test_custom':
                ...
                wr.writerow([scene_name, test_psnr, test_ssim ])
