...
def get_train_data(args, max_t_step_size):
    ...
    elif args.dataset == 'ATD12k' and args.phase == 'train':
        data_train = ATD12k(args, Train=True)
...
def get_test_data(args, multiple, validation):
    ...
    if args.phase == 'test_stopmotion':
        data_test = Stopmotion_Test(args) 
    elif args.dataset == 'X4K1000FPS'  and args.phase == 'test':
        data_test = X_Test(args, multiple, validation)  # 'validation' for validation while training for simplicity
    elif args.dataset == 'Vimeo' and args.phase != 'test_custom' and args.phase !='test_adobe':
        data_test = Vimeo_Test(args, validation)
    elif args.dataset == 'ATD12k' and args.phase != 'test_custom':
        data_test = ATD12k(args, Train=False)
    elif args.phase == 'test_custom':
        data_test = Custom_Test(args, multiple)
    elif args.phase == 'test_middlebury':
        data_test = Middlebury_other(args, '/HDD/dataset/MiddleBurySet/other-data', '/HDD/dataset/MiddleBurySet/other-gt-interp')
    elif args.phase == 'test_snufilm':
        data_test = SNUFILM(args, '/HDD/dataset/SNU_FILM/images/test', mode=args.mode)
    elif args.phase == 'test_adobe':
        data_test = Adobe_Test(args, 8)
...
def channel_change_net(flow, channel):
    _,C,H,W = flow.shape
    conv = nn.Conv2d(C, channel, 1)
    conv = conv.cuda()
    flow = conv(flow)
    return flow
...
def make_2D_dataset_Adobe(dir, multiple, t_step_size):
    """ make [I0,I1,It,t,scene_folder] """
    """ 1D (accumulated) """
    testPath = []
    t = np.linspace((1 / multiple), (1 - (1 / multiple)), (multiple - 1))
    for scene_folder in sorted(glob.glob(os.path.join(dir, '*', ''))):  # [scene1,scene2,..]
        frame_folder = sorted(glob.glob(scene_folder + '*.png'))  # 32 multiple, ['00000.png',...,'00032.png']
        for idx in range(0, len(frame_folder), t_step_size):  # 0,32,64,...
            if idx == len(frame_folder) - 1:
                break
...
class X_Test(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frames = frames_loader_test(self.args, I0I1It_Path, self.validation) #output이 뭔지 찾기
        ...
        return frames, np.expand_dims(np.array(t_value, dtype=np.float32), 0), scene_name, [ I0_path, It_path, I1_path]
...
class Adobe_Test(data.Dataset):
    def __init__(self, args, multiple):
        ...
        self.testPath = make_2D_dataset_Adobe(self.args.test_data_path, multiple, t_step_size=8)
    ...
    def __getitem__(self, idx):
        ...
        frames = frames_loader_test(self.args, I0I1It_Path, validation=False) #output이 뭔지 찾기
        # including "np2Tensor [-1,1] normalized"
...
class Middlebury_other(data.Dataset):
    def __init__(self, args, input_dir, gt_dir):
        ...
        self.args = args
        self.framesPath= []
        for scene in self.im_list:
            tmp = []
            tmp.append(os.path.join(input_dir, scene, 'frame10.png'))
            tmp.append(os.path.join(input_dir, scene, 'frame11.png'))
            tmp.append(os.path.join(gt_dir, scene, 'frame10i11.png'))
            self.framesPath.append(tmp)
        
        self.num_scene = len(self.framesPath)
        
        if len(self.framesPath) == 0:
            raise (RuntimeError("found 0 files in subfolders of "+ input_dir + "\n"))
        else:
            print('# of Middlebury testset:', self.num_scene)

    def __getitem__(self, idx):
        scene_name = self.im_list[idx]
...
class Stopmotion_Test(data.Dataset):
    def __init__(self, args):
        self.args = args
        self.framesPath = []
        self.frames_list = sorted(os.listdir(self.args.stopmotion_data_path)) # 이부분 확인해볼 필요가 있음. 
        self.num_scene = len(self.frames_list)  # total test scenes
        if len(self.frames_list) == 0:
            raise (RuntimeError("Found 0 files in subfolders of: " + args.stopmotion_data_path + "\n"))
        else:
            print("# of Vimeo triplet testset : ", self.num_scene)

    def __getitem__(self, idx):
        frame_element = self.frames_list[idx]
        frame_elements = sorted(os.listdir(os.path.join(self.args.stopmotion_data_path, frame_element)))

        scene_name = os.path.join(frame_element)
        print('scene_name'+scene_name)
        #scene_name = os.path.join(scene_name[-3], scene_name[-2])
        I0 = os.path.join(self.args.stopmotion_data_path, frame_element,  frame_elements[0])
        It = os.path.join(self.args.stopmotion_data_path, frame_element, frame_elements[1])
        I1 = os.path.join(self.args.stopmotion_data_path, frame_element, frame_elements[2])
        I0I1It_Path = [I0, I1, It]
        frames = frames_loader_test(self.args, I0I1It_Path, validation=False)
...
def _make_dataset(dir):
    ...
    return framesPath, framesIndex, framesFolder


class ATD12k(data.Dataset):

    def __init__(self, args, Train=False, transform=None):

        root = args.atd12k_data_path
        
        if Train:
            root = os.path.join(root, 'train_10k')
        else:
            root = os.path.join(root, 'test_2k_original')
        
        self.framesPath, self.framesIndex, self.framesFolder= _make_dataset(root)
        self.Train = Train
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        if self.Train == True:
            ...
            frames = frames_loader_test(self.args, self.framesPath[index], validation=False)
           
          
        scene_name = self.framesPath[index][0].split(os.sep) 
        scene_name = os.path.join(scene_name[-3], scene_name[-2])
        I0, It, I1 = self.framesPath[index]  
        I0_path = I0.split(os.sep)[-1]
        I1_path = I1.split(os.sep)[-1]
        It_path = It.split(os.sep)[-1]
...
def make_2D_dataset_Custom_Test(dir, multiple):
    ...
    for scene_folder in sorted(glob.glob(os.path.join(dir, '*', ''))):  # [scene1, scene2, scene3, ...]
        frame_folder = sorted(os.listdir(scene_folder), key=lambda x: int(os.path.splitext(x)[0])) # ex) ['00000.png',...,'00123.png']
        for idx in range(0, len(frame_folder)):
            if idx == len(frame_folder) - 1:
                break
            for suffix, mul in enumerate(range(multiple - 1)):
                I0I1It_paths = []
                I0I1It_paths.append(os.path.join(scene_folder,frame_folder[idx]))  # I0 (fix)
                I0I1It_paths.append(os.path.join(scene_folder,frame_folder[idx + 1]))  # I1 (fix)
                target_t_Idx = os.path.join(scene_folder,frame_folder[idx]).split(os.sep)[-1].split('.')[0]+'_' + str(suffix).zfill(3) + '.png'
                ...
                I0I1It_paths.append(os.path.join(scene_folder,frame_folder[idx]).split(os.path.join(dir, ''))[-1].split(os.sep)[0])  # scene1
                testPath.append(I0I1It_paths)
    return testPath

class SNUFILM(data.Dataset):
    def __init__(self, args, data_root, mode='hard'):
        
        self.args = args
        
        test_root = os.path.join(data_root, 'test')
        test_fn = os.path.join('/HDD/dataset/SNU_FILM/annotations', 'test-%s.txt' % mode)
    ...
    def __getitem__(self, index): #need to modify as XVFINet format 
        
        #Use self.test_all_images:
        imgpaths = self.frame_list[index]
        scene_name = imgpaths[1].split(os.sep) #전처리 output 참고 
        scene_name = os.path.join(scene_name[3], scene_name[4], scene_name[5])
        frames = []

        for path in imgpaths:   
            img = Image.open(os.path.join('/HDD/dataset/SNU_FILM',path))
            frames.append(img)
        ...
        frames = RGBframes_np2Tensor(frames, self.args.img_ch)
        
        return frames, np.expand_dims(np.array(0.5, dtype=np.float32), 0), scene_name, imgpaths # output 
...
def metrics_evaluation_Adobe(pred_save_path, test_data_path, metrics_types, flow_flag=False, multiple=8, server=None):
    """
    ...
    pred_framesPath = []
    for scene_folder in sorted(glob.glob(os.path.join(pred_save_path, '*', ''))):  # [scene1,scene2,..]
        scene_framesPath = []
        for frame_path in sorted(glob.glob(scene_folder + '*.png')):
            #scene_framesPath.append(frame_path)
            pred_framesPath.append(frame_path)
    ...
    for scene_idx, scene_folder in enumerate(pred_framesPath):
        ...
        output_img = cv2.imread(pred_candidate).astype(np.float32)  # BGR, [0,255]
        target_img = cv2.imread(pred_candidate.replace(pred_save_path, test_data_path)).astype(
                    np.float32)  # BGR, [0,255]
        pred_frame_split = pred_candidate.split(os.sep)
        msg = "[x%d] frame %s, " % (
        ...
        if "tOF" in keys:  # tOF
            ...
            pre_out_grey = cv2.cvtColor(cv2.imread(pred_candidate).astype(np.float32),
    ...
    for key in keys:
        ...
        print("%s, total frames %d, total avg %02.4f" %
