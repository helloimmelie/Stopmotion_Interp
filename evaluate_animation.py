...
def save_flow_to_img(flow, des):
        f = flow.data.cpu().numpy().transpose([1, 2, 0])
        fcopy = f.copy()
        fcopy[:, :, 0] = f[:, :, 1]
        fcopy[:, :, 1] = f[:, :, 0]
...
def validate(config):   
    ...
    # prepare model
    model = XVFInet(config).cuda()
    retImg = []

    # load weights
    dict1 = torch.load(config.checkpoint)
    model.load_state_dict(dict1['state_dict_Model'], strict=False)
    ...
    with torch.no_grad():
        ...
        for validationIndex, validationData in enumerate(validationloader, 0):
            ...
            config.divide = 2**(config.S_tst) * config.module_scale_factor * 4
            ...
            for tt in range(config.inter_frames):
                ...
                images = torch.stack([I1, I2], dim=2)
                H_padding = (config.divide - H % config.divide) % config.divide
                W_padding = (config.divide - W % config.divide) % config.divide
                
                if H_padding != 0 or W_padding != 0:
                    images = F.pad(images, (0, W_padding, 0, H_padding), "constant")
            
                t_value = np.linspace((1 / config.multiple), (1 - (1 / config.multiple)), (config.multiple - 1))
                
                t_value = torch.Tensor(t_value).unsqueeze(0).cuda()
                
                outputs, flow_l = model(images, t_value, is_training=False )
                ...
                save_flow_to_img(flow_l[0,:2,:,:].cpu(), store_path + '/' + folder[1][0] + '/' + index[1][0] + '_flow')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config',default='configs/config_test_w_sgm.py')
