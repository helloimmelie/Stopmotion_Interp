...
class VFInet(nn.Module):
    ...
    def forward(self, x, feat_x, flow_l_prev, t_value, level, is_training):
        ...
        # Flow upscale
        up = nn.Upsample(size = (feat_x.shape[2], feat_x.shape[3]*self.scale, feat_x.shape[4]*self.scale), mode='nearest')
        feat_x = up(feat_x)
        ...
        #flow_integrated = torch.cat((flow_t0_l,flow_t1_l),dim=1) #여기서부터 체크 
        #flow_integrated = upsample2d_flow_as(
                #flow_integrated, feat_x, mode="bilinear", if_rate=True)
        #flow_s =  self.self_guided_upsample(
                #flow_up_bilinear=flow_integrated.detach(), feature_1= feat_x[:,:,0,:,:], feature_2=feat_x[:,:,1,:,:])
